...
if __name__ == '__main__':
    ...
    eo_analysis.visualize_roc(postprocessed['test'], save = f"{FIGURE_DIRECTORY}/post_processed/fig_roc_seed_{str(seed).zfill(3)}.png")
    ...
    roc.visualize_metric(test_df, save = f"{FIGURE_DIRECTORY}/roc/fig_seed_{str(seed).zfill(3)}.png")

    precision_recall = FairnessMetricVisualizer(metric = 'precision_recall', parameters = config)
    precision_recall.visualize_metric(test_df, save = f"{FIGURE_DIRECTORY}/precision_recall/fig_seed_{str(seed).zfill(3)}.png")

    calibration = FairnessMetricVisualizer(metric = 'calibration', parameters = config)
    calibration.visualize_metric(test_df, save = f"{FIGURE_DIRECTORY}/calibration/fig_seed_{str(seed).zfill(3)}.png")